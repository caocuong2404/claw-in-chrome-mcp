...
import type { ClawInChromeContext } from "./types.js";
...
interface ToolRequest {
  ...
  params?: {
    client_id?: string; // "desktop" | "ai-ide"
    tool?: string;
...
class McpSocketClient {
  ...
  constructor(context: ClawInChromeContext) {
  ...
  private async validateSocketSecurity(socketPath: string): Promise<void> {
    ...
    try {
      ...
      const isSocketDir =
        dirBasename.startsWith("claw-in-chrome-mcp-browser-bridge-") ||
        dirBasename.startsWith("claude-mcp-browser-bridge-");
      if (isSocketDir) {
        try {
